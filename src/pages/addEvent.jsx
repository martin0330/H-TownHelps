...
const EventManage = () => {
    ...
    const fetchAvailableUsers = async (date) => {
        try {
            const response = await fetch(
                `http://localhost:5000/api/availableUsers?date=${date.toISOString()}`
            );
            if (!response.ok) {
    ...
    const onSubmit = async (data) => {
        try {
            data.skills = [];
            data.volunteers = [];
            console.log(data); // This will now include the correct date value
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className='mb-4'>
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <DatePicker
                                    selected={field.value} // Pass field.value to DatePicker
                                    onChange={(date) => field.onChange(date)} // Use field.onChange to update the form state
                                    className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline'
